...
contract("JCream crDai mainnet", function (accounts) {
    ...
    it("DAI total Supply sent to user1", async function () {
        ...
        console.log("UnBlockedAccount DAI balance: " + web3.utils.fromWei(await daiContract.methods.balanceOf(UnBlockedAccount).call()) + " DAI");
        // expect(web3.utils.fromWei(result.toString(), "ether")).to.be.equal(MYERC20_TOKEN_SUPPLY.toString());
        await daiContract.methods.transfer(user1, web3.utils.toWei("100")).send({
            from: UnBlockedAccount
        })
        console.log("UnBlockedAccount DAI balance: " + web3.utils.fromWei(await daiContract.methods.balanceOf(UnBlockedAccount).call()) + " DAI");
        console.log("user1 DAI balance: " + web3.utils.fromWei(await daiContract.methods.balanceOf(user1).call()) + " DAI");
    });
    ...
    it("user1 buys some token daiTrA", async function () {
        ...
        console.log("price tranche A: " + web3.utils.fromWei(trAPrice));

        trParams = await jCreamContract.trancheAddresses(1);
        expect(trParams.buyerCoinAddress).to.be.equal(DAI_ADDRESS);
        console.log("User1 DAI balance: " + web3.utils.fromWei(await daiContract.methods.balanceOf(user1).call(), "ether") + " DAI");

        tx = await daiContract.methods.approve(jCreamContract.address, web3.utils.toWei("10", "ether")).send({
            from: user1
        });
        tx = await jCreamContract.buyTrancheAToken(1, web3.utils.toWei("10", "ether"), {
            from: user1
        });

        console.log("User1 New DAI balance: " + web3.utils.fromWei(await daiContract.methods.balanceOf(user1).call(), "ether") + " DAI");
        console.log("User1 trA tokens: " + web3.utils.fromWei(await daiTrAContract.balanceOf(user1), "ether") + " DTA");
        console.log("JCream DAI balance: " + web3.utils.fromWei(await daiContract.methods.balanceOf(jCreamContract.address).call(), "ether") + " DAI");
    ...
    it('transferring Tranche A tokens should also transfer staking details', async function () {
        ...
        console.log("user1 trA Balance: " + web3.utils.fromWei(bal1) + " DTA")
        stkDetails = await jCreamContract.stakingDetailsTrancheA(user1, 1, 1);
        ...
        await daiTrAContract.transfer(user2, web3.utils.toWei("200"), {
            from: user1
        });
        bal1 = await daiTrAContract.balanceOf(user1)
        bal2 = await daiTrAContract.balanceOf(user2)
        ...
        stkDetails = await jCreamContract.stakingDetailsTrancheA(user1, 1, 1);
        console.log("user1 stkDetails, startTime: " + stkDetails[0].toString() + ", amount: " + stkDetails[1].toString())
        console.log("user2 trA Balance: " + web3.utils.fromWei(bal2) + " DTA")
    ...
    it("user1 buys some token daiTrB", async function () {
        ...
        tx = await jCreamContract.buyTrancheBToken(1, web3.utils.toWei("10", "ether"), {
            from: user1
        });
    ...
    it('transferring Tranche B tokens should also transfer staking details', async function () {
        ...
        console.log("user1 trB Balance: " + web3.utils.fromWei(bal1) + " DTB")
        stkDetails = await jCreamContract.stakingDetailsTrancheB(user1, 1, 1);
        console.log("user1 stkDetails, startTime: " + stkDetails[0].toString() + ", amount: " + stkDetails[1].toString())
        bal2 = await daiTrBContract.balanceOf(user2)
        expect(bal2.toString()).to.be.equal("0")

        await daiTrBContract.transfer(user2, web3.utils.toWei("5"), {
            from: user1
        });
        bal1 = await daiTrBContract.balanceOf(user1)
        bal2 = await daiTrBContract.balanceOf(user2)
        ...
        stkDetails = await jCreamContract.stakingDetailsTrancheB(user1, 1, 1);
        console.log("user1 stkDetails, startTime: " + stkDetails[0].toString() + ", amount: " + stkDetails[1].toString())
        console.log("user2 trB Balance: " + web3.utils.fromWei(bal2) + " DTB")
        stkDetails = await jCreamContract.stakingDetailsTrancheB(user2, 1, 1);
        console.log("user2 stkDetails, startTime: " + stkDetails[0].toString() + ", amount: " + stkDetails[1].toString())
    });
    ...
    it("user1 redeems token daiTrA", async function () {
        oldBal = web3.utils.fromWei(await daiContract.methods.balanceOf(user1).call(), "ether");
        ...
        newBal = web3.utils.fromWei(await daiContract.methods.balanceOf(user1).call(), "ether");
    ...
    it("user2 redeems token daiTrA", async function () {
        oldBal = web3.utils.fromWei(await daiContract.methods.balanceOf(user2).call(), "ether");
        console.log("User1 Dai balance: " + oldBal + " DAI");
        ...
        newBal = web3.utils.fromWei(await daiContract.methods.balanceOf(user2).call(), "ether");
        console.log("User1 New Dai balance: " + newBal + " DAI");
        bal = await daiTrAContract.balanceOf(user2);
        console.log("User1 trA tokens: " + web3.utils.fromWei(bal, "ether") + " DTA");
        console.log("User1 trA interest: " + (newBal - oldBal) + " DAI");
    ...
    describe('higher percentage for test coverage', function() {
        it('calling unfrequently functions', async function () {    
          ...
          await jCreamContract.setTrancheAFixedPercentage(1, web3.utils.toWei("0.03", "ether"))
    
          await jCreamContract.getTrancheACurrentRPS(1)
    
          await jCreamContract.setTrAStakingDetails(1, user1, 1, 0, 1634150567)
          await jCreamContract.getSingleTrancheUserStakeCounterTrA(user1, 1)
          await jCreamContract.getSingleTrancheUserSingleStakeDetailsTrA(user1, 1, 1)
    
          await jCreamContract.setTrBStakingDetails(1, user1, 1, 0, 1634150567)
          await jCreamContract.getSingleTrancheUserStakeCounterTrB(user1, 1)
          await jCreamContract.getSingleTrancheUserSingleStakeDetailsTrB(user1, 1, 1)
