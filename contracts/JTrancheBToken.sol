...
contract JTrancheBToken is Ownable, ERC20, AccessControl, IJTrancheTokens {
	...
	function _transfer(address from, address to, uint256 value) internal override {
        ...
        if (rewClaimCompleted && value > 0) {
			...
			uint256 stkDetNum = IJCream(jCreamAddress).getSingleTrancheUserStakeCounterTrB(from, protTrancheNum);
			for (uint256 i = 1; i<= stkDetNum; i++){
				(tempTime, tempAmount) = IJCream(jCreamAddress).getSingleTrancheUserSingleStakeDetailsTrB(from, protTrancheNum, protTrancheNum);
				if (tempAmount > 0) {
					if (tempAmount <= tempValue) {
						IJCream(jCreamAddress).setTrBStakingDetails(protTrancheNum, from, i, 0, tempTime);
						IJCream(jCreamAddress).setTrBStakingDetails(protTrancheNum, to, i, tempAmount, block.timestamp);
						tempValue = tempValue.sub(tempAmount);
					} else {
						uint256 remainingAmount = tempAmount.sub(tempValue);
						IJCream(jCreamAddress).setTrBStakingDetails(protTrancheNum, from, i, remainingAmount, tempTime);
						IJCream(jCreamAddress).setTrBStakingDetails(protTrancheNum, to, i, tempValue, block.timestamp);
						tempValue = 0;
					}
				}
				if (tempValue == 0)
